...
import { Server, Cpu, HardDrive, Thermometer, Box } from 'lucide-react';
...
function MemoryBar({ percent, used, total }: { percent: number; used: number; total: number }) {
  ...
  return (
    <div className="space-y-1.5">
      <div className="flex items-center justify-between text-[10px]">
        ...
        <span className="text-white/50 tabular-nums">{formatBytes(used)} / {formatBytes(total)}</span>
      </div>
...
export function ServerNodeCard({ server, data, index }: ServerNodeCardProps) {
  ...
  const { system, containers, docker } = data;
    ...
    >
      <div className={clsx(
        ...
        <div className="relative z-10 p-5">
          ...
          <div className="flex items-center gap-3 mb-4">
            <div className="relative">
              ...
              <span className={clsx(
                'absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 rounded-full border-2 border-[#0a0a1a]',
                isOnline ? 'bg-emerald-400 status-pulse-healthy' : 'bg-red-400'
              )} />
            </div>
          ...
          {!hasData ? (
            ...
            <>
              ...
              {/* RAM Bar */}
              <MemoryBar percent={memPercent} used={memUsed} total={memTotal} />
        ...
        <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none"
          style={{
            background: `radial-gradient(circle at 50% 50%, ${isOnline ? 'rgba(16,185,129,0.04)' : 'rgba(239,68,68,0.04)'}, transparent 70%)`
          }}
        />
      </div>
    </motion.div>
